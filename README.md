...
Signal K runs well on a Raspberry Pi 4B or 5, with dependencies on how many other devices are connected and sending data, such as running a NEMA 2000 backbone. The built-in wireless on the Pi has numerous adjustment necessary to get it to work properly, such as disabling auto power down. This is especially the case when it's running in Access Point mode. A fundamental problem is the minimal antenna. A infinitely better solution is to use a small travel wifi travel router, connected to the Pi with an ethernet cable. Of course this is more equipment to maintain. The Pi 5 is runs very fast with an NVMe drive, but they are not easy to get to work reliably. Achieving good working onboard wifi and an NVMe drive can be frustrating. 
